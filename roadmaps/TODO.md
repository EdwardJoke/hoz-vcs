...
|---|---------|------|---------------|-------------------|
| 1 | **`commit`** | [commit.zig#L42-L50](src/cli/commit.zig#L42) | Hardcoded `"abc123"` tree OID, hardcoded author, **never writes to object store, never updates HEAD** | Read index → build tree → write tree object → write commit object with real tree OID + parent + timestamp → update HEAD ref |
| 2 | **`add`** | [add.zig#L58-L62](src/cli/add.zig#L58) | Prints `"Added 'path'"`, **never hashes files, never writes blobs, never updates index** | Read file content → SHA-1 hash → write loose object → create IndexEntry (mode, oid, stage) → write/update index file |
| 3 | **`log`** | [log.zig#L42-L65](src/cli/log.zig#L42) | All formats print identical fake data: `"abc123 Initial commit"` — **never reads `.git/` at all** | Walk HEAD → parse commit objects → resolve parent chain → format with real OIDs/authors/dates/messages; support `--oneline`, `--format=`, `-n` limit |
