...
pub const Fsck = struct {
    ...
    errors: std.ArrayListUnmanaged(FsckError),
    warnings: std.ArrayListUnmanaged(FsckError),
    ...
    pub const ErrorType = enum { missing_object, corrupted_object, invalid_type, loose_not_pack, missing_ref };
...
test "Fsck init" {
    const fsck = try Fsck.init(std.testing.allocator);
    defer fsck.deinit();
    try std.testing.expect(!fsck.hasErrors());
}

test "Fsck checkObject" {
    var fsck = try Fsck.init(std.testing.allocator);
    defer fsck.deinit();
